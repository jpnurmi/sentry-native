...
static void
configure_scope_callback(sentry_scope_t *scope, void *closure)
{
    char **argv = (char **)closure;
    int argc = 0;
    while (argv[argc] != NULL) {
        argc++;
    }

    sentry_scope_clear_breadcrumbs(scope);
...
main(int argc, char **argv)
{
    ...
    if (has_arg(argc, argv, "capture-with-scope")) {
        ...
        sentry_capture_event_with_scope(event, configure_scope_callback, argv);
    }
