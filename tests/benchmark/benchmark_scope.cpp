...
static void
benchmark_scope_event(benchmark::State &state)
{
    ...
    for (auto _ : state) {
        ...
        sentry_value_t event
            = new_event(static_cast<size_t>(state.range(0)));
        state.ResumeTiming();
...
BENCHMARK(benchmark_scope_event)
    ->Arg(0)
    ->Arg(20)
    ->Arg(100)
...
static void
benchmark_scope_transaction(benchmark::State &state)
{
    sentry_options_t *options = sentry_options_new();
    sentry_scope_t *scope = new_scope();
    ...
    for (auto _ : state) {
        ...
        sentry_value_t transaction
            = new_transaction(static_cast<size_t>(state.range(0)));
        state.ResumeTiming();

        sentry__scope_apply_to_event(
            scope, options, transaction, SENTRY_SCOPE_ALL);
...
BENCHMARK(benchmark_scope_transaction)
    ->Arg(0)
    ->Arg(20)
    ->Arg(100)
    ->Unit(benchmark::kMillisecond);
